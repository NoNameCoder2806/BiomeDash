...
void renderCredits(GameState& game, SDL_Renderer* renderer)
{
	...
	for (const auto& label : game.creditLines)
	{
		...
		// Original position from loading phase
		glm::vec2 originalPos = label->getPosition();

		// Apply scrolling offset
		float renderY = originalPos.y - game.creditsScrollOffset;
		...
		if (visible)
		{
			...
			label->setPosition(originalPos.x, originalPos.y);
		}
	}
}